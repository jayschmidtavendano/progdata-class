...
##A function to select columns from data set. 
##first argument is which dataframe, then elaborate which columns we want to select.
##This outputs the variables(columns) in the order they are listed. 
```
...
##everything = selecting everything
##We stated the specific variable we wanted first (continent) then we used everything() to call everything else
```
...
##you can do this by in select say new name = old name
...
```{r}
arrange(gapminder, -year)
##This arranges by year, the minus tells it go from largest to smallest
```
